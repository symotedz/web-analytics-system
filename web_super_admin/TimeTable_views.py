...
def TimeTable_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/super_admin/%20TimeTables_detail/')
    else:
        form = TimeTableForm(request.POST)
    context = {
        'form' : form
    }
    return render(request, 'Dashboard/TimeTable_create.html', context)
...
def TimeTable_update(request, pk):
    ...
    return render(request, 'Dashboard/TimeTable_update.html', context)
