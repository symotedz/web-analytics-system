...
def class_block_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/web_admin/class_blocks_detail/')
    else:
        form = Class_blockForm(request.POST)
    context = {
        'form' : form
    }
    return render(request, 'webadmin/class_block_create.html', context)
...
def class_blocks_detail(request):
    ...
    return render(request, 'webadmin/class_blocks_detail.html', context)
...
def class_block_detail(request, pk):
    ...
    return render(request, 'webadmin/class_block_detail.html', context)
...
def class_block_update(request, pk):
    ...
    return render(request, 'webadmin/class_block_update.html', context)
