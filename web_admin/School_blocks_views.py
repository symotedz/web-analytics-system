...
def School_blocks_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/web_admin/School_blockss_detail/')
    else:
        form = School_blocksForm(request.POST)
    context = {
        'form' : form
    }
    return render(request, 'webadmin/School_blocks_create.html', context)
...
def School_blockss_detail(request):
    ...
    return render(request, 'webadmin/School_blockss_detail.html', context)
...
def School_blocks_detail(request, pk):
    ...
    return render(request, 'webadmin/School_blocks_detail.html', context)
...
def School_blocks_update(request, pk):
    ...
    return render(request, 'webadmin/School_blocks_update.html', context)
