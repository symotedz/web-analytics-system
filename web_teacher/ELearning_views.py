...
def ELearning_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/super_admin/%20ELearnings_detail/')
    else:
        form = ELearningForm(request.POST)
    context = {
        'form' : form
    }
    return render(request, 'Dashboard/ELearning_create.html', context)
...
def ELearning_update(request, pk):
    ...
    return render(request, 'Dashboard/ELearning_update.html', context)
