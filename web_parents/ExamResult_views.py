...
from web_super_admin.models import ExamResult
from web_super_admin.forms import ExamResultForm
...
def ExamResult_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/super_admin/ExamResults_detail/')
    else:
        form = ExamResultForm(request.POST)
    context = {
        'form' : form
...
def ExamResults_delete(request):
    ...
    Examresult.delete()
    return redirect('/super_admin/ExamResult_detail/')
